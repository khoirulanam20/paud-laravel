...
<head>
    ...
    <style>
        ...
        .guest-nav { background: #fff; box-shadow: 0 2px 12px rgba(0,0,0,0.08); position: sticky; top: 0; z-index: 100; }
        .guest-nav-link { font-weight: 700; color: #4B5563; transition: color .2s; padding: .5rem .75rem; border-radius: 12px; }
        .guest-nav-link:hover, .guest-nav-link.active { color: #FF8C42; background: #FFF3E0; }
        .hero-section { background: linear-gradient(135deg, #FFF3E0 0%, #FFF0F5 50%, #F0F4FF 100%); }
        .btn-cta { display: inline-flex; align-items: center; gap: .5rem; background: var(--guest-orange); color: #fff; font-weight: 800; font-size: 1rem; padding: .85rem 2rem; border-radius: 2rem; box-shadow: 0 4px 16px rgba(255,140,66,.45); transition: transform .2s, box-shadow .2s; border: none; cursor: pointer; text-decoration: none; }
        .btn-cta:hover { transform: translateY(-2px); box-shadow: 0 8px 24px rgba(255,140,66,.5); color: #fff; }
        ...
        .section-wave { width: 100%; line-height: 0; }
        ...
        .footer-guest { background: #1F2937; color: #D1D5DB; }
        /* Floating shapes */
        ...
        @media (max-width: 640px) {
            ...
            .mobile-menu-open .guest-nav-links { display: flex; flex-direction: column; position: fixed; inset: 0; top: 64px; background: white; padding: 1.5rem; z-index: 99; gap: .5rem; }
        }
    </style>
</head>
<body>
    ...
    <nav class="guest-nav" x-data="{ open: false }">
        <div class="max-w-6xl mx-auto px-4 sm:px-6 flex justify-between items-center h-16">
            ...
            <div class="hidden sm:flex items-center gap-3">
                @auth
                    <a href="{{ route('dashboard') }}" class="btn-cta-outline text-sm px-4 py-2">Dashboard</a>
                @else
            ...
            <!-- Mobile Hamburger -->
            <button @click="open=!open" class="sm:hidden p-2 rounded-xl" style="color:#FF8C42;">
        ...
        <!-- Mobile Menu -->
        <div x-show="open" x-transition class="sm:hidden border-t px-4 py-4 space-y-1 bg-white" style="display:none;">
    ...
    <footer class="footer-guest mt-0 pt-12 pb-8">
        <div class="max-w-6xl mx-auto px-4 sm:px-6">
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-10 pb-10 border-b border-gray-700">
                <div>
                    <div class="flex items-center gap-2 mb-3">
                        <div class="h-10 w-10 rounded-2xl flex items-center justify-center text-xl" style="background: linear-gradient(135deg,#FFD93D,#FF8C42);">🌈</div>
                        <span style="font-family:'Baloo 2',sans-serif; font-size:1.2rem; font-weight:800; color:#FFD93D;">PAUD Kita</span>
                    </div>
                    <p class="text-sm text-gray-400 leading-relaxed">{{ $cms['footer_text'] ?? 'Tumbuh bersama, bahagia bersama. 💛' }}</p>
                </div>
                <div>
            ...
            <p class="text-center text-xs text-gray-500 mt-6">© {{ date('Y') }} PAUD Manager. Dibuat dengan ❤️ untuk pertumbuhan anak Indonesia.</p>
        </div>
    </footer>
</body>
</html>
