@php
    ...
    $pendingCount = 0;
    if ($user->hasRole('Admin Sekolah') && $user->sekolah_id) {
                ...
                <div class="hidden sm:flex min-w-0 flex-1 items-center gap-1 overflow-x-auto overscroll-x-contain [scrollbar-gutter:stable]">
                    ...
                    <a href="{{ route($item['route']) }}"
                       class="nav-item flex shrink-0 items-center gap-1.5 whitespace-nowrap {{ request()->routeIs($item['pattern']) ? 'active' : '' }}">
                        ...
                        <span class="inline-flex items-center justify-center h-4.5 min-w-[1.15rem] px-1 rounded-full text-white text-[10px] font-bold leading-none" style="background:#FF8C42;">{{ $item['badge'] }}</span>
                        @endif
                    </a>
                    @endforeach
                </div>
