                            ...
                            <tr class="hover:bg-gray-50/30 transition">
                                ...
                                <td class="px-6 py-4 text-center">
                                    <div class="flex items-center justify-center gap-2">
                                        <a href="{{ route((auth()->user()->hasRole('Admin Sekolah') ? 'admin.' : 'pengajar.').'master-kegiatan-rutin.show', $master) }}" @if($loop->first) data-tour="pg-master-rutin-action-detail" @endif class="h-8 w-8 rounded-lg bg-gray-50 flex items-center justify-center hover:bg-blue-500 hover:text-white transition group border border-gray-100 shadow-sm text-blue-600" title="Detail Capaian Siswa">
                                            <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
                                        </a>
                                        @if(!auth()->user()->hasRole('Pengajar'))
                                        <a href="{{ route((auth()->user()->hasRole('Admin Sekolah') ? 'admin.' : 'pengajar.').'master-kegiatan-rutin.edit', $master) }}" @if($loop->first) data-tour="pg-master-rutin-action-edit" @endif class="h-8 w-8 rounded-lg bg-gray-50 flex items-center justify-center hover:bg-yellow-500 hover:text-white transition group border border-gray-100 shadow-sm text-yellow-600" title="Edit Kegiatan">
                                        ...
                                        <form action="{{ route((auth()->user()->hasRole('Admin Sekolah') ? 'admin.' : 'pengajar.').'master-kegiatan-rutin.destroy', $master) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data ini?');" class="inline-block">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" @if($loop->first) data-tour="pg-master-rutin-action-delete" @endif class="h-8 w-8 rounded-lg bg-gray-50 flex items-center justify-center hover:bg-red-500 hover:text-white transition group border border-gray-100 shadow-sm text-red-600" title="Hapus Kegiatan">
